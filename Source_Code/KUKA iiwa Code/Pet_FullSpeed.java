 ...
 * @see #dispose()
 */
public class AnimalFast extends RoboticsAPIApplication {
	@Inject
	private LBR lbr;
		...
		while(times>0){
			try {
				...
				//Attack
					...
					lbr.move(ptp(getApplicationData().getFrame("/Pet/bAttackSmall")));
					lbr.move(ptp(getApplicationData().getFrame("/Pet/aIdle")));
				}
				else if (info.equals("ba")) {
					logger.info("BigAttack");
					lbr.move(ptp(getApplicationData().getFrame("/Pet/cAttackBig")));
					lbr.move(ptp(getApplicationData().getFrame("/Pet/aIdle")));
				}
				...
				//Food
					...
					lbr.move(ptp(getApplicationData().getFrame("/Pet/fEat")));	
					lbr.move(ptp(getApplicationData().getFrame("/Pet/aIdle")));
				}
				else if (info.equals("bf")) {
					logger.info("BigEat");
					lbr.move(ptp(getApplicationData().getFrame("/Pet/kEatBig")));
					lbr.move(ptp(getApplicationData().getFrame("/Pet/aIdle")));
				}
				...
				//Jump
					...
					logger.info("BigJump");
					lbr.move(ptp(getApplicationData().getFrame("/Pet/eJumpNew")));
					lbr.move(ptp(getApplicationData().getFrame("/Pet/aIdle")));
				}
				else if (info.equals("sj")) {
					logger.info("SmallJump");
					lbr.move(ptp(getApplicationData().getFrame("/Pet/dJumpSmall")));
					lbr.move(ptp(getApplicationData().getFrame("/Pet/aIdle")));
				}
